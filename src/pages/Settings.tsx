...
export const Settings: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTab === 'billing' && user && hasActiveSubscription) {
      fetchBillingHistory()
    }
  ...
  const tabs = [
    ...
    { id: 'email' as TabType, label: 'Email Preferences', icon: Mail },
    { id: 'notifications' as TabType, label: 'Notifications', icon: Bell },
    { id: 'billing' as TabType, label: 'Billing', icon: CreditCard },
    { id: 'calling' as TabType, label: 'Auto-Calling', icon: Phone },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl">
        ...
        <div className="relative backdrop-blur-sm bg-white/10 border border-white/20 rounded-3xl p-8 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold drop-shadow-sm">Settings</h1>
      ...
      {/* Message Banner */}
      {message && (
        <div className={`relative backdrop-blur-sm rounded-2xl p-4 border ${
        ...
        <div className="lg:col-span-3">
          <div className="relative backdrop-blur-sm bg-white/60 dark:bg-gray-800/60 border border-white/20 dark:border-gray-700/50 rounded-2xl p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white/50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-2">
                      Email changes require verification. Contact support@ratemonitorpro.com to update your email.
                    </p>
                  </div>
            ...
            {activeTab === 'email' && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Email Preferences</h2>
                  <p className="text-gray-600 dark:text-gray-400">Choose what emails you want to receive</p>
                </div>

                <div className="space-y-4">
                  {[
                    { key: 'rate_alerts', label: 'Rate Alerts', desc: 'Get notified when rates hit your clients\' targets' },
                    { key: 'weekly_reports', label: 'Weekly Reports', desc: 'Summary of rate changes and opportunities' },
                    { key: 'client_updates', label: 'Client Updates', desc: 'Notifications about client activity' },
                    { key: 'market_insights', label: 'Market Insights', desc: 'Industry news and market analysis' },
                    { key: 'product_updates', label: 'Product Updates', desc: 'New features and improvements' }
                  ].map((pref) => (
                    <div key={pref.key} className="flex items-center justify-between p-4 bg-white/40 dark:bg-gray-700/40 rounded-xl">
                      <div>
                      ...
                      <button
                        onClick={() => setEmailPrefs({ ...emailPrefs, [pref.key]: !emailPrefs[pref.key as keyof EmailPreferences] })}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          emailPrefs[pref.key as keyof EmailPreferences]
                            ? 'bg-blue-600'
                      ...
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                            emailPrefs[pref.key as keyof EmailPreferences] ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
                  ...
                  <Button onClick={handleEmailPrefsSave} loading={loading}>
                    <Save className="w-4 h-4 mr-2" />
                    Save Preferences
                  </Button>
                </div>
            ...
            {activeTab === 'notifications' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400">Manage your in-app notifications</p>
                </div>

                <div className="space-y-4">
                  {[
                    { key: 'in_app_notifications', label: 'In-App Notifications', desc: 'Show notifications in the app' },
                    { key: 'rate_alerts', label: 'Rate Alert Notifications', desc: 'Notify when client target rates are reached' },
                    { key: 'call_results', label: 'Call Results', desc: 'Updates about automated calling outcomes' },
                    { key: 'system_updates', label: 'System Updates', desc: 'Important system announcements' }
                  ].map((pref) => (
                    <div key={pref.key} className="flex items-center justify-between p-4 bg-white/40 dark:bg-gray-700/40 rounded-xl">
                      <div>
                        <p className="font-medium text-gray-900 dark:text-gray-100">{pref.label}</p>
                      ...
                      <button
                        onClick={() => setNotificationPrefs({ ...notificationPrefs, [pref.key]: !notificationPrefs[pref.key as keyof NotificationPreferences] })}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          notificationPrefs[pref.key as keyof NotificationPreferences]
                            ? 'bg-blue-600'
                            : 'bg-gray-200 dark:bg-gray-700'
                        }`}
                      >
                        <span
                          className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                            notificationPrefs[pref.key as keyof NotificationPreferences] ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
                  ...
                  <Button onClick={handleNotificationPrefsSave} loading={loading}>
                    <Save className="w-4 h-4 mr-2" />
                    Save Preferences
                  </Button>
                </div>
              ...
              <div className="space-y-6">
                ...
                {subscriptionLoading ? (
                  ...
                  <>
                    ...
                    <div className="bg-white/40 dark:bg-gray-700/40 rounded-xl p-6 border border-gray-200/50 dark:border-gray-600/50">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1">
                            {hasActiveSubscription ? currentProduct?.name || 'Active Subscription' : 'No Active Subscription'}
                          </h3>
                          <p className="text-gray-600 dark:text-gray-400">
                            {hasActiveSubscription ? (
                              isTrialing ? (
                                ...
                                `Status: ${subscription?.status} • Renews ${subscription?.current_period_end ? formatDate(new Date(subscription.current_period_end * 1000).toISOString()) : 'N/A'}`
                              )
                            ) : (
                        ...
                        <div className="text-right">
                          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                            ${hasActiveSubscription ? currentProduct?.price || '49' : '0'}
                          </p>
                        ...
                        {loadingBilling ? (
                          ...
                          <div className="space-y-3">
                            {billingHistory.map((bill) => (
                              <div key={bill.id} className="flex items-center justify-between p-4 bg-white/40 dark:bg-gray-700/40 rounded-xl">
                                <div>
            ...
            {activeTab === 'calling' && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Auto-Calling Preferences</h2>
                  <p className="text-gray-600 dark:text-gray-400">Configure automated calling settings</p>
                </div>

                <div className="relative overflow-hidden bg-gradient-to-br from-purple-50/60 to-blue-50/60 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200/50 dark:border-purple-700/50 rounded-xl p-8 text-center">
                  <div className="absolute top-0 right-0 bg-gradient-to-r from-purple-600 to-blue-600 text-white px-4 py-1 rounded-bl-xl text-sm font-medium">
                    Coming Soon
                  </div>
                  <Phone className="w-16 h-16 mx-auto mb-4 text-purple-600 dark:text-purple-400" />
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">
                    Automated Calling with Bland AI
                  </h3>
                  <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                    Configure call schedules, scripts, and preferences for automated client outreach. 
            ...
            {activeTab === 'password' && (
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Change Password</h2>
            ...
            {activeTab === 'danger' && (
              <div className="space-y-6">
                ...
                <div className="bg-red-50/60 dark:bg-red-900/20 border border-red-200/50 dark:border-red-700/50 rounded-xl p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-600 dark:text-gray-400 mb-4">
                        ...
                        all your clients, rates data, and cancel your subscription. Please be certain.
                      </p>
                      <Button 
                        variant="outline"
