// src/components/RateMonitor/RateMonitorCard.tsx - MOVED FROM CRM + HOT/COLD BADGES
import React from 'react'
import { Phone, Mail, Edit, Trash2, TrendingUp, Calendar, AlertCircle, Flame, Snowflake, AlertTriangle } from 'lucide-react'
import { TargetProgressBar } from '../CRM/TargetProgressBar'
...
interface Mortgage {
  ...
  last_contact?: string // ← ADD THIS for hot/cold badge
}
...
export const RateMonitorCard: React.FC<RateMonitorCardProps> = ({ 
  ...
  const getContactStatus = () => {
    const days = getDaysSinceContact(mortgage.last_contact)
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center space-x-1">
            {/* 🔥 NEW: Hot/Cold Badge */}
            {contactStatus.show && (
              <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold border ${contactStatus.bgColor} ${contactStatus.textColor} ${contactStatus.borderColor} backdrop-blur-sm`}>
                <contactStatus.icon className={`w-3 h-3 ${contactStatus.iconColor}`} />
