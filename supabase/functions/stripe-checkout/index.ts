...
Deno.serve(async (req) => {
  try {
    ...
    console.log('Creating checkout session for:', { price_id, mode, user: user?.id });
    ...
    if (!customer || !customer.customer_id) {
      ...
      // Handle trial customers - create real Stripe customer if it's a trial_ customer
      if (customerId.startsWith('trial_')) {
        ...
        console.log(`Upgrading trial customer ${customerId} to real Stripe customer ${newCustomer.id}`);

        // Update the customer_id in our database
      ...
      if (mode === 'subscription') {
        // Check if subscription exists for existing customer
